...
def enhance(img):
    ...
    # clipLimit은 너무 세게 올려서 노이즈까지 도드라지는 걸 막는 상한임.
    # 2.0에서 3.0으로 올렸음. 새로 라벨링한 455장에서 +1.68%포인트 (개선 12 / 퇴행 4).
    # 기존 449장에서는 오히려 0.37%포인트 떨어지는데, 그쪽은 개발하면서 실패 사례를
    # 계속 들여다본 집단이라 우리 파서가 거기에 맞춰져 있어서임. 처음 보는 사진 기준으로
    # 판단하는 게 맞다고 보고 3.0을 골랐음.
