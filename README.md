...
- **실측 결과** (팀 손라벨 904장. `predict.ipynb`를 그대로 실행해 측정. 2026-09-25 재측정, 라벨 3건 교정 반영. 오프라인 동작은 아래 리허설로 따로 확인):

  | 집합 | n | 부분점수 | 완전일치 | 응답률 |
  |---|---|---|---|---|
  | 전체 | 904 | **92.92%** | 89.71% | 97.3% |
  | A–C (파서·규칙이 맞춰짐 / 검출기 학습 제외) | 449 | 94.43% | 91.09% | 98.4% |
  | D+E (튜닝 판단용 / 검출기 학습 포함) | 303 | 91.75% | 88.78% | 95.7% |
