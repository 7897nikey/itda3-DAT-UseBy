...
- **실측 결과** (팀 손라벨 904장. `predict.ipynb`를 그대로 실행해 측정. 2026-09-26 재측정, 라벨 교정 13건 반영. 오프라인 동작은 아래 리허설로 따로 확인):

  | 집합 | n | 부분점수 | 완전일치 | 응답률 |
  |---|---|---|---|---|
  | 전체 | 904 | **93.18%** | 90.27% | 97.3% |
  | A–C (파서·규칙이 맞춰짐 / 검출기 학습 제외) | 449 | 94.80% | 91.76% | 98.4% |
  | D+E (튜닝 판단용 / 검출기 학습 포함) | 303 | 91.97% | 89.44% | 95.7% |
  | **F (판단에 안 씀 / 검출기 학습 포함)** | 152 | **90.79%** | 87.50% | 97.4% |

  **속도는 채점 환경과 같은 규격에서 직접 쟀습니다.** GitHub Actions 의 `ubuntu-22.04`
  러너가 x86_64 4코어라 운영진 환경(Ubuntu 22.04 LTS / x86_64 / 4-Core / RAM 8GB)과
  CPU 규격이 같습니다. 실행 로그는 `.github/workflows/verify-x86.yml` 의 실행 결과로
  남습니다.

  | 측정 | 값 |
  |---|---|
  | x86_64 4코어 (AMD EPYC 7763) 실측 | 장당 1.863초 (custom_data 97장) |
  | 같은 이미지 arm64 실측 | 장당 0.874초 |
  | → x86 / arm64 배율 | **2.13배** |
  | 대회 이미지 904장 arm64 실측 | 장당 1.183초 |
  | **→ x86 환산 500장 예상** | **1,261초 (예산 2,500초의 50.4%)** |
  | 최대 메모리 | **1.23GB** (제한 8GB 의 15%) |

  개발 머신이 Apple Silicon 이라 로컬 측정치를 그대로 쓰면 절반 이하로 과소평가됩니다.
  `custom_data` 는 긴 변 중앙값이 2,048px 로 대회 원본(1,196px)보다 커서 거기서 나온
  장당 시간 자체는 상한이고, 배율만 취해 대회 이미지 실측에 곱했습니다.
  ...
  **개선폭은 F 기준으로 읽어 주세요.** 인식 모델 교체로 전체 904건이 84.66% 에서
  ...
  ### 한 번도 보지 않은 294장에서 재확인했습니다

  본선 기간 동안 팀이 새로 손라벨한 300장(연사본 6건 제외 294장)입니다.
  세 명이 100장씩 나눠 달았고(`splits/holdout_G/`), 어떤 파라미터·설계 결정에도
  쓰지 않았습니다. **여기서 딱 한 번 열었습니다.**
  ...
  | 완전일치 | 74.06% | **84.30%** |
  | 응답률 | 88.74% | **93.86%** |
  | 연 / 월 / 일 정확도 | 84.6 / 79.2 / 77.1% | **90.1 / 88.7 / 86.7%** |

  **차이 +8.19%p, 95% 신뢰구간 [+4.66, +11.83], 개선 45 / 퇴행 9,
  부호검정 p=0.000001.** 부호검정과 부트스트랩이 둘 다 통과한 유일한 결과입니다.

  새 라벨 294장의 연도 분포는 기존 904장과 거의 같습니다
  (2021: 39.4% 대 39.4%, 2022: 20.2% 대 20.8%, 2026: 19.5% 대 18.5%).
  뽑을 때 이미 라벨된 904장과 겹치는 것, 그리고 기존 평가셋의 연사본으로
  의심되는 것(지각 해시 거리 20 이하)을 제외했습니다.

  **보고할 숫자는 전체 90.01%입니다.** 다만 어느 집합에도 오염이 하나씩 있어서,
  그 성격을 밝혀 둡니다.

  | 집합 | 검출기가 학습에 썼나 | 파서·규칙을 여기에 맞췄나 |
  |---|---|---|
  | A–C 449장 | **아니오** (450건 전량 학습에서 제외) | **예** — 실패 사례를 반복해 열어 보며 고침 |
  | D+E 303장 | **예** | 일부 — 파라미터 판단에 씀 |
  | F 152장 | **예** | 아니오 — 최종 확인 1회만 |

  검출기를 학습할 당시에는 손라벨이 A–C 밖에 없어서 그 450건을 학습에서 뺐고
  (`yolo/holdout_eval_ids.csv`), D–F 는 그 뒤에 라벨링돼서 제외 목록에 들어가지
  못한 채 학습에 포함됐습니다. 배치별 실측은 A/B/C 가 0/150·0/150·0/149,
  D/E/F 가 149/152·148/151·150/152 입니다.

  **그래서 "한 번도 안 쓴 구간"이라는 표현은 파서·규칙 기준으로만 참입니다.**
  검출 단계까지 포함해 완전히 독립인 평가 집합은 현재 없고, 새로 확보하는 중입니다.
  다만 개선 전후를 짝지어 비교할 때는 양쪽이 같은 검출기를 쓰므로 이 오염이
  상쇄되어, **변화량(Δ) 측정에는 영향이 거의 없습니다.** 영향을 받는 것은
  절대 점수이고, 그만큼 낙관적으로 읽힐 수 있습니다.

  규약은 [`splits/README_holdout.md`](splits/README_holdout.md)에 있고
  `split_eval.py`가 이를 강제합니다.

- **2자리 연도 표기의 자리 판정**: `24/12/21` 은 연-월-일로 읽으면 2024-12-21,
  일-월-연으로 읽으면 2021-12-24 이고 둘 다 달력상 유효해서 숫자만으로는 구분되지
  않습니다. 예전에는 무조건 연-월-일을 골랐는데, 남은 실패 122건 중 9건이 전부
  이 지점이었습니다. 손라벨의 연도 분포가 심하게 치우쳐 있다는 점을 씁니다 —
  촬영이 2025년 하반기라 2021·2022(지난 것)와 2026(살아 있는 것)에 몰리고
  2016·2017·2030 은 거의 없습니다. 두 해석의 사전확률이 6배 이상 벌어질 때만
  뒤집습니다. 분포는 A–C + D+E 741건에서만 뽑았고 배치 F 와 홀드아웃은 뺐습니다.

  904건에서 개선 9 / 퇴행 2, +0.48%p (부트스트랩 CI [+0.04, +0.96], 부호검정 p=0.065).
  **유의하다고 말하지 않고 "방향이 일관되어 채택했다"로 적습니다.** 퇴행 2건은 정답이
  실제로 2024년(분포의 0.67%)이라 사전이 틀린 쪽으로 민 경우로, 사전 기반 규칙의
  원리적 비용입니다.

- **영문 기한 표기 인식**: 앵커 키워드에 `BEST BEFORE` / `BEST BY` / `USE BY` /
  `SELL BY` / `BBE` 를 정규식으로 추가했습니다. OCR 이 단어 사이를 점으로 읽거나
  (`BEST.BY`), 글자를 흘리거나(`SELL BY` → `SEL BY`), 대소문자가 섞여도 잡힙니다.

  이게 없어서 공백으로 끊긴 날짜(`BEST.BY 2021 12 02`)를 통째로 놓치고 있었습니다.
  공백 구분 패턴 자체는 원래 있었지만 기한 키워드 ±30자 안에서만 도는데,
  영문 표기가 앵커로 인정되지 않아 한 번도 발동하지 못했습니다. 같은 이유로
  날짜 종류 분류기도 `Use By: 04 JUL 21 … 같은제조` 에서 뒤쪽 `제조` 만 보고
  제조일자로 오판했습니다. 분류기에도 같이 연결했습니다.

- **영문 월 + 2자리 연도**: 기존 영문 월 패턴 3종이 전부 4자리 연도만 받아서
  `FEB/21/21`, `APR/29/21`, `04 JUL 21` 을 놓치고 있었습니다. 2자리 연도판을
  추가하고, 구분자에 `/` 를 넣었습니다(`2022/JUN/14`). 월 이름 3글자가 정확히
  일치해야 하는 강한 앵커라 구분자를 넓혀도 오탐 위험은 낮습니다.

  904건에서 개선 20 / 퇴행 7, +1.51%p (p=0.0192). **배치 F 에서는 개선 3 /
  퇴행 0** 입니다. 앵커를 넓혔는데도 오탐이 늘지 않은 것이 핵심이고,
  응답률이 95.6% → **97.0%** 로 오른 것이 실체입니다.
