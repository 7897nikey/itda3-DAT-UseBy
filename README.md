...
- **영역 탐지**: YOLO11n — 라벨 데이터(학습 2,892장 / 검증 808장)로 파인튜닝한 4클래스(date/due/code/full) 검출기. **팀이 직접 라벨링한 손라벨 450장은 학습에서 완전히 제외**하고 성능 측정 전용으로만 씀(학습 데이터 유출 방지). 검증 808장 기준 평균 mAP50 0.916. 원래 `.pt`(torch)로 내보냈던 걸 `region_best.onnx`로 다시 내보내 RapidOCR과 같은 onnxruntime 엔진으로 통일함 — torch 의존성이 완전히 사라지고 검출 속도가 7.1배 빨라짐(검출 결과는 동일).
...
- **실측 결과** (팀 손라벨 904장, 학습에 안 쓴 독립 검증셋. 네트워크를 끊은 상태에서 `predict.ipynb`를 그대로 실행해 측정):

  | 집합 | n | 부분점수 | 완전일치 | 응답률 |
  |---|---|---|---|---|
  | 전체 | 904 | **85.07%** | 80.53% | 91.8% |
  | A–C (개발에 노출됨) | 449 | 89.09% | 84.41% | 95.8% |
  | D+E (튜닝 판단용) | 303 | 79.43% | 74.26% | 87.5% |
  | **F (한 번도 보지 않음)** | 152 | **84.43%** | 81.58% | 88.8% |

  속도는 장당 0.56–0.63초 — 500장 환산 시 예산 2400초의 12–13%.

  **보고할 숫자는 전체 85.07%입니다.** 다만 A–C 449장은 개발 과정에서 실패 사례를
  반복해 들여다보며 규칙을 고친 집단이라 점수가 부풀려져 있습니다. 그래서 배치 F를
